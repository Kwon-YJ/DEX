...
class UniswapV2Utils(object):
    ...
    def pair_for(factory, token_a, token_b):
        ...
        return Web3.to_checksum_address(Web3.toHex(raw)[-40:])
...
class UniswapV2Client(UniswapObject):
    ...
    def __init__(self, address, private_key, provider=None):
        ...
        self.contract = self.conn.eth.contract(
            address=Web3.to_checksum_address(UniswapV2Client.ADDRESS), abi=UniswapV2Client.ABI)
        self.router = self.conn.eth.contract(
            address=Web3.to_checksum_address(UniswapV2Client.ROUTER_ADDRESS), abi=UniswapV2Client.ROUTER_ABI)
    ...
    def _is_approved(self, token, amount=MAX_APPROVAL_INT):
        erc20_contract = self.conn.eth.contract(
            address=Web3.to_checksum_address(token), abi=UniswapV2Client.PAIR_ABI)
    ...
    def approve(self, token, max_approval=MAX_APPROVAL_INT):
        ...
        erc20_contract = self.conn.eth.contract(
            address=Web3.to_checksum_address(token), abi=UniswapV2Client.ERC20_ABI)
    ...
    def get_pair(self, token_a, token_b):
        ...
        """
        addr_1 = self.conn.to_checksum_address(token_a)
        addr_2 = self.conn.to_checksum_address(token_b)
    ...
    def get_token_0(self, pair):
        ...
        pair_contract = self.conn.eth.contract(
            address=Web3.to_checksum_address(pair), abi=UniswapV2Client.PAIR_ABI)
